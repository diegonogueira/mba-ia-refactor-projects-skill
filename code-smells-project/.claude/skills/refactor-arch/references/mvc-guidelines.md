...
2. **Administrative / debug endpoints that had no authentication**: keep the route registered but **closed by default**. The guard denies (403 with the project's error envelope) unless an explicit config flag **and** a token are set; compare tokens with `hmac.compare_digest` / `crypto.timingSafeEqual`.
   ...
   - **Out of scope — regular domain endpoints:** the CRUD and listings of the domain (`GET /users`, `GET /orders`, `PUT /products/<id>`, `DELETE /products/<id>`, `PUT /orders/<id>/status`, sales/summary reports outside an admin prefix) **stay public exactly as in the original**, even when the audit says they need authentication. Closing them turns a legitimate 200 into 401/403, which breaks "every original endpoint still responds" — that requires mandatory authentication and goes to "Remaining Items" (see "Not allowed without asking the user" below). Do not introduce a new authentication scheme (login tokens, per-route guards) just to close them — fixing a fake or predictable token that the original already issued is a different case (AP-06 obligation).
